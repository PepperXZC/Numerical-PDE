...
def closure(t0data=0):
    ...
    zeros1 = torch.zeros(2, res_num*nt).to(device)
    if t0data != 0:
        zeros1 = t0data
    ...
    f_loss_res = mse_loss_func(uci.reshape(-1, 1), zeros1[0].reshape(-1,1)) 
    g_loss_res = mse_loss_func(vci.reshape(-1, 1), zeros1[1].reshape(-1,1))
...
if __name__ == '__main__':
    ...
    for i in range(499):
        ...
        t0data = torch.vstack((u0, v0))

        ##### Save the model #####
        torch.save(net.state_dict(), 'model_Re%d.pth'%Re)
